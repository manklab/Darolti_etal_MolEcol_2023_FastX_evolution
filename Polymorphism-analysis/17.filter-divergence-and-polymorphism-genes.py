...
parser.add_argument("outfile", type=str,
					help="An outfile containing target gene name, t, N, S, dN/dS, dN, dS, N*dN, S*dS, N*pN, S*pS")
	...
	with open(divergence_file, "r") as divergence:
		...
		for line in divergence:
			...
			if orthogroup in orthogroup_gene:
				...
				gene_divergence[gene].append(t)
				gene_divergence[gene].append(N)
				gene_divergence[gene].append(S)
				gene_divergence[gene].append(dN_dS)
				gene_divergence[gene].append(dN)
				gene_divergence[gene].append(dS)
...
def main():
	...
	print "Number of genes with polymorphsim data =", len(gene_polymorphism) 

	with open(args.outfile, "w") as out:
		out.write("Gene,t,N,S,dN/dS,dN,dS,N*dN,S*dS,N*pN,S*pS")
		out.write("\n")
