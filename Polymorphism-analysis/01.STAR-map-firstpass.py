...
def main():
    ...
    for infile in infiles:
        ...
        directory1 = os.path.dirname(infile) + "/firstpass/"
        directory2 = os.path.dirname(infile) + "/firstpass/" + uniqid
    ...
    for uniqid in filedictionary:
        for file in filedictionary[uniqid]:
            ...
            directory = os.path.dirname(file)+"/firstpass/"+uniqid+"/"
            if file.endswith("R1.fastq.gz"):
                forward_fastq = file
            elif file.endswith("R2.fastq.gz"):
                reverse_fastq = file
        command = ["STAR --genomeDir "+args.STARindex+" --readFilesCommand zcat --readFilesIn "+forward_fastq+" "+reverse_fastq+" --runThreadN 12 --outFilterMultimapNmax 1 --outFileNamePrefix "+directory]
